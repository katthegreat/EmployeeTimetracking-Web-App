...
export default function Dashboard() {
  ...
  const [auth, setAuth] = useState(() => {
    // Safely parse auth data from localStorage
    try {
  ...
  // Redirect if not logged in and load data
  useEffect(() => {
    if (!auth) {
      navigate('/');
      return;
    ...
    // Load employee time records (example for non-admin users)
    if (auth.role !== 'admin') {
  ...
  // If no auth (should be caught by useEffect redirect)
  if (!auth) return null;
